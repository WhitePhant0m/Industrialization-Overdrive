...
@EventBusSubscriber(value = Dist.CLIENT, modid = ExEI.ID, bus = EventBusSubscriber.Bus.MOD)
public class ExEIClient {
    @SubscribeEvent
    ...
    private static void registerBlockEntityRenderers(FMLClientSetupEvent event) {
        for (DeferredHolder<Block, ? extends Block> blockDef : ExEIBlocks.Registry.BLOCKS.getEntries()) {
